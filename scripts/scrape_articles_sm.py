"""Scrape WOL articles by docId (Samoan ↔ English), paragraph-aligned.

Uses Docker curl-impersonate for fetching (browser-like TLS fingerprint).

Usage:
    uv run python scripts/scrape_articles_sm.py --pilot              # 5 articles
    uv run python scripts/scrape_articles_sm.py --pub lv             # all chapters from pub "lv"
    uv run python scripts/scrape_articles_sm.py --docids 1102008070 1102015820
    uv run python scripts/scrape_articles_sm.py --library            # crawl entire WOL library
    uv run python scripts/scrape_articles_sm.py --library-cat tusi   # single category
"""
...
WOL_PUB_TOC_URL = "https://wol.jw.org/sm/wol/publication/r173/lp-sm/{pubCode}"
...
SECTION_RE = re.compile(r"/wol/publication/r173/lp-sm/([^/]+)/\d+")
# Regex for library sub-pages
LIBRARY_RE = re.compile(r"/wol/library/r173/lp-sm/")
...
def harvest_docids_from_library(url: str, max_depth: int = 5,
    ...
    for a_tag in article.find_all("a", href=True):
        ...
        # Library sub-page (same category — must be at or below current path)
        if LIBRARY_RE.search(href) and href not in _visited:
            full_url = WOL_BASE + href if href.startswith("/") else href
            # Prevent upward traversal: only follow links whose path is
            # at least as deep as the current URL's path.
            cur_path = urlparse(url).path.rstrip("/")
            sub_path = urlparse(full_url).path.rstrip("/")
            if sub_path.startswith(cur_path) and full_url not in _visited:
...
def harvest_all_library() -> list[str]:
    """Harvest docIds from all categories in the WOL Samoan library."""
    # Samoan WOL library category slugs (from wol.jw.org/sm/wol/library/r173/lp-sm/lomiga-uma)
    categories = [
        ("Watchtower", "le-olomatamata"),
        ("Awake!", "ala-mai"),
        ("Books", "tusi"),
        ("Meeting", "polokalame-mo-le-sauniga"),
        ("Ministry", "faiva-o-le-malo"),
        ("Brochures", "polosiua-ma-tama\u02BBitusi"),
        ("Tracts", "s\u0101vali"),
        ("Programs", "polokalame"),
        ("Series", "faasologa-o-mataupu"),
        ("Research Aids", "fesoasoani-mo-sa\u02BBili\u02BBiliga"),
    ]
    ...
    for name, slug in categories:
        url = f"{WOL_BASE}/sm/wol/library/r173/lp-sm/lomiga-uma/{slug}"
        print(f"\nCrawling {name} ({slug})...")
...
def is_metadata_paragraph(text: str) -> bool:
    ...
    t = text.strip()
    # Chapter/section headers: "CHAPTER 7", "MATAUPU 7", "SECTION 3", etc.
    if re.match(r"^(CHAPTER|MATAUPU(\s+E)?|SECTION|PART|IUNITE)\s+\d+$", t, re.IGNORECASE):
        return True
    # Copyright lines
    ...
    if t.lower().startswith("photo credit"):
        return True
    # Page/chapter index lines like "236Mataupu" or "PAGECHAPTER"
    if re.match(r"^\d+[A-Z]", t) and len(t) < 40:
        return True
    if t in ("PAGECHAPTER", "MATAUPUITULAU"):
        return True
    return False
...
def align_paragraphs(sm_paras: list[tuple[str, str]],
                     en_paras: list[tuple[str, str]],
                     doc_id: str,
                     pub_code: str | None) -> list[dict]:
    """Align SM and EN paragraphs and return list of aligned pair dicts."""
    sm_url = WOL_ARTICLE_URL.format(docId=doc_id, **SM_BUNDLE)
    en_url = WOL_ARTICLE_URL.format(docId=doc_id, **EN_BUNDLE)

    sm_pids = [pid for pid, _ in sm_paras]
    en_pids = [pid for pid, _ in en_paras]

    sm_dict = {pid: text for pid, text in sm_paras}
    en_dict = {pid: text for pid, text in en_paras}

    pairs = []

    # Check alignment quality
    common_pids = [pid for pid in sm_pids if pid in en_dict]
    sm_count = len(sm_paras)
    en_count = len(en_paras)

    if sm_count == 0 or en_count == 0:
        return []

    mismatch_ratio = abs(sm_count - en_count) / max(sm_count, en_count)

    if mismatch_ratio > 0.2 and len(common_pids) < min(sm_count, en_count) * 0.8:
        # Fall back to document-level alignment
        sm_text = " ".join(text for _, text in sm_paras)
        ...
        if sm_count == en_count and sm_pids == en_pids:
            confidence = 0.9
        else:
            confidence = 0.8
        ...
        for pid in common_pids:
            ...
            if sm_chars < 20 and en_chars < 20:
                continue

            # Skip extreme ratio pairs (likely pid misalignment)
            if en_chars > 0:
                ratio = sm_chars / en_chars
...
def scrape_article(doc_id: str, pub_code: str | None = None) -> list[dict] | None:
    ...
    # Extract paragraphs
    sm_paras = extract_paragraphs(sm_html)
    en_paras = extract_paragraphs(en_html)

    if not sm_paras or not en_paras:
        print(f"  SKIP {doc_id} -- no paragraphs extracted "
              f"(SM: {len(sm_paras)}, EN: {len(en_paras)})")
        return None

    # Align
    pairs = align_paragraphs(sm_paras, en_paras, doc_id, pub_code)
    return pairs


def main():
    parser = argparse.ArgumentParser(
        description="Scrape WOL articles by docId (Samoan ↔ English), paragraph-aligned.")
    ...
    output_file = ALIGNED_DIR / "articles_sm.jsonl"

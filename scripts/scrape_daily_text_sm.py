"""Scrape WOL daily text pages (Samoan ↔ English), date-aligned.

Uses Docker curl-impersonate for fetching (browser-like TLS fingerprint).

Usage:
    uv run python scripts/scrape_daily_text_sm.py --year 2025
    uv run python scripts/scrape_daily_text_sm.py --range 2025-03-01 2025-03-05
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Scrape WOL daily text pages (Samoan ↔ English), date-aligned.")
    ...
    output_file = ALIGNED_DIR / "daily_text_sm.jsonl"
    ...
    extracted_from_page = {}  # date_iso -> (sm_text, en_text) extracted from adjacent fetch
    ...
    with open(output_file, "a") as out:
        for dt in tqdm(dates_to_scrape, desc="Scraping daily text"):
            ...
            if dt_iso in extracted_from_page:
                sm_text, en_text = extracted_from_page[dt_iso]
            else:
                # Fetch Samoan page
                sm_url = make_daily_url(SM_BUNDLE, dt)
                raw_sm_path = str(RAW_DIR / "wol_sm" /
                ...
                for offset in [-1, 1, 2]:
                    ...
                    if (adj_iso not in existing_dates and
                        ...
                        adj_sm = extract_daily_text(sm_html, adj_dt)
                        adj_en = extract_daily_text(en_html, adj_dt)
                        if adj_sm and adj_en:
                            extracted_from_page[adj_iso] = (adj_sm, adj_en)

            if not sm_text or not en_text:
                tqdm.write(f"  SKIP {dt_iso} -- extraction failed "
                           f"(SM: {'ok' if sm_text else 'empty'}, "
